...
2. Or tap **Advantage Pro** → **Owner unlock** (or **Owner unlock** on Coming soon) and enter the code.
3. Or set `localStorage` key `advantage.proUnlocked` to `1`.

Unlocked home shows an active Pro card and **Open Pro toolbox**. The code lives in `src/lib/proUnlock.ts` as `PRO_UNLOCK_CODE` (default `advantage`). Override at build time with `VITE_PRO_UNLOCK_CODE`. Lock again with **Lock Pro** on home, `?pro=0`, or by clearing that flag.
