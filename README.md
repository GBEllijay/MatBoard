...
Public testers land on three product cards. **Advantage White** is the live path (Live Bout + Rounds on `/white`). **Advantage Coach** and **Advantage Pro** stay grayed and open Coming soon ads (Coach-angled vs gym-owner — not checkout). `/slideshow`, `/screensaver`, `/tournament`, `/suite`, `/competitors`, `/instructors`, `/rankings`, `/techniques`, `/technique-tree`, `/schedule`, `/roster`, `/coach`, and `/pro` go to a Coming soon page unless the matching product is on (Coach or Pro for Coach tools; Pro only for `/pro`, `/schedule`, `/slideshow`, `/suite`, `/competitors`, `/instructors`, and `/rankings`).
