...
- **Advantage Coach** — Coming soon on public home. When unlocked (`?coach=gbellijay`), **Open Coach** goes to `/coach` with four tools only: **Mock Tournament**, **Competitor roster** (Competitor Management System — local-only, no CSV), **Daily Lesson Plan** (today's class plan on this phone: coach name, intro, warm-up note, technique blocks with optional water breaks, cool-down, and closing), and **Daily Training Videos** (up to 10 on-device clips with loop + drill timer; **Add clips** opens a chooser: **Record** for the phone camera in video mode, or **Pick from gallery**; media stays on this phone). Coach does not open Gallery or Gym Owner Console folders (Pro Shop, Events, Class Schedule, slideshow). Public deep links stay gated.
